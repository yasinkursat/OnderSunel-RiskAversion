...
parameter (beta = 0.953d+0, sigma = 2d+0, r = 0.017d+0, &
          ...
          d0 = -0.69d+0, d1 = 0.80d+0, prob_excl_end1 = 0.0d+0, phi = 0.0d+0, theta = 0.62d+0, share = (theta/(1d+0 - theta))**(1d+0/sigma))
...
objective_excl_w = - u_fun_w(output) - beta*exp_v_next 
    ...
    do i=2,period_num
        ...
        if (excl(i-1,j) < 0.5) then !Country was not excluded in the previous period
            if (v_def+1d-5>=v_no_def) then  !COUNTRY DEFAULTS
                ...
                !if (q(i,j)<0.9) then
                !    WRITE(nout, '(G15.6, X, F12.8, X, F12.8, X, F12.8, X, F12.8, X, F12.8)')  v_no_def-v_def, q(i,j), b_next, v_def, v_valor
                !end if
...
indicator_external = 0 !FROM EXTERNAL FILE       ! THIS IS TO TAKE THE INITIAL GUESS FROM AN OUTSIDE FILE ( FROM PREVIOUS ITERATIONS )
