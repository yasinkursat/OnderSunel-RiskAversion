...
load data_sim_dss.txt
load def_per_dss.txt
load param_dss.txt

data_sim = data_sim_dss;
def_per = def_per_dss;

per_num = param_dss(1);  %HOW MANY PERIODS IN EACH SAMPLE
n = param_dss(2);        %HOW MANY SAMPLES
...
c_wholesample = data_sim(:,5);% zeros(length(data_sim),1);
...
y_default = zeros(num_observations,1);
c_default = zeros(num_observations,1);
tb_default = zeros(num_observations,1);
ra_default = zeros(num_observations,1);
b_default = zeros(num_observations,1);
...
for i=1:n
       for j=1:num_observations_vector(i)
            %ra_vector = (1./q(def_per_matrix(j,i) - sample_size:def_per_matrix(j,i) - 1,i).^4)/1.017^4-1;
            ...
            ra_default(index_observation) = (1./q(def_per_matrix(j,i)-1,i).^4)/1.017^4-1;
%             ra_default = (1./qdef(def_per).^4) - 1.017^4;


num_simulations = min(length(std_y_vector), 2000);
