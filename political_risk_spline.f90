...
DOUBLE PRECISION :: beta, sigma, r, b_inf, b_sup, y_inf, y_sup, eps_inf, eps_sup, std_eps,&
                    rho, mean_y, std_y, pi_number, lambda, zero, width, prob_excl_end, delta, scale_y, criteria,&
...
parameter (beta = 0.953d+0, sigma = 2d+0, r = 0.017d+0, &
           std_eps = 0.025d+0, rho = 0.945d+0, mean_y = -0.5d+0*std_eps**2,  pi_number = 3.1415926535897932d+0, prob_office = 0.90d+0, &
          lambda = 0.969d+0, zero = 0d+0,  width = 1.5d+0, prob_excl_end = 0.282d+0,  delta = 1.0d+0, & !0.045d+0, &
          cdf_inf = 3.167d-5, cdf_sup = 1d+0 - 3.167d-5, scale_y = 10d+0, &
          d0 = -0.69d+0, d1 = 0.80d+0, prob_excl_end1 = 0.0d+0, phi = 0.0d+0, theta = 0.5d+0, share = (theta/(1d+0 - theta))**(1d+0/sigma))
...
parameter (b_num= 100, y_num = 60, quad_num_v = 100, eps_num = quad_num_v, quad_num_q = 100, cdf_num = 100, b_num_finer = 100, y_num_finer = 100)
...
!open (13, FILE='graphs\im_grid.txt',STATUS='replace')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (y_threshold < y_min) then !THRESHOLD TOMORROW IS LOW
    ...
    elseif (y_threshold > y_max) then !THRESHOLD TOMORROW IS HIGH
...
USE param
INTEGER :: d
DOUBLE PRECISION :: y_valor, b_valor, b_valor_def, v_valor, convergence, deviation, q_fun, b_next, g,&
...
parameter (sample_num = 50, period_num=801)
DOUBLE PRECISION :: random_matrix(period_num, sample_num, 3), random_vector(1:3*sample_num*period_num), &
                    ...
                    v_valor, v0_fun, v1_fun, DNORIN, q_menu_fun
...
do j=1,sample_num   !SOLVE FOR SAMPLE j
    ...
    do i=2,period_num
        ...
        if (excl(i-1,j) < 0.5) then !Country was not excluded in the previous period
            if (v_def + 1d-5 >= v_no_def) then  !COUNTRY DEFAULTS
                d(i,j) = 2
                i_default_global = 2
                excl(i,j) = 1
                b(i+1,j) = 0d+0
                ...
                y(i,j) = scale_y * MIN(lambda, exp(z(i,j)))!EXP(z(i,j)) * (1-d0) - d1*EXP(z(i,j))**2d+0
                ...
                !q(i,j) = q_menu_fun(b_next, z(i,j))
                if (q(i,j)<0.9) then
                    WRITE(nout, '(G15.6, X, F12.8, X, F12.8, X, F12.8, X, F12.8, X, F12.8, X, F12.8)')  v_no_def-v_def, q(i,j), b_next, v_def, v_valor,  q_menu_fun(b_next, z(i,j))
                end if
            ...
            if (random_matrix(i,j,2) <= prob_excl_end) then !exclusion ends today
                ...
                !b_global = b_next
                q(i,j) = 1d+0 / (delta + r) !q_fun(b_next, z(i,j))
                y(i,j) = scale_y * MIN(lambda, exp(z(i,j)))!EXP(z(i,j)) * (1-d0) - d1*EXP(z(i,j))**2d+0  !output loss while exclusion
                c(i,j) = y(i,j)! - q(i,j)* b(i+1,j)
                tb(i,j) = y(i,j) - c(i,j)
            end if
        end if
...
if (indicator_external < 0.5) then
    ...
    do i_b = 1,b_num
        do i_y = 1,y_num
            ...
            READ(18, '(F15.10, X, F15.10, X, F15.10, X, F15.10)') w_matrix(i_b, i_y), &
            w0_matrix(i_b, i_y), w1_matrix(i_b, i_y), w_excl_matrix(i_b, i_y)
